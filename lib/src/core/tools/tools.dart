...
import 'package:inspireui/utils/regex_utils.dart';
...
class Tools {
  static bool isBuilder = false;
  static List<String> externalAppURLs = [];
  static List<String> externalNonBrowserAppURLs = [];
  static List<Currency> currenciesData = [];

  /// Setup the tools with the given parameters.
  /// Required to call back if the application changes the config in env.dart
  static void setup({
    required List<String> externalAppURLs,
    required List<String> externalNonBrowserAppURLs,
    required List<Currency> currenciesData,
    required bool isBuilder,
  }) {
    Tools.externalAppURLs = externalAppURLs;
    Tools.externalNonBrowserAppURLs = externalNonBrowserAppURLs;
    Tools.currenciesData = currenciesData;
    Tools.isBuilder = isBuilder;
  }
  ...
  static Future<bool> launchURL(
    ...
    final url = prepareURL(originUrl);
    ...
    // Check if can launch url in external app, not external browser.
    // If not be able to launch, try to open it in external app
    ...
    if (!isLaunched) {
      printError('Could not launch $originUrl');
    }
  ...
  static bool needToOpenExternalApp(String? url) {
    for (var app in externalNonBrowserAppURLs) {
      if (url?.startsWith(app) ?? false) {
        return true;
      }
    }
    for (var app in externalAppURLs) {
  ...
  static bool isRTL(BuildContext context) {
    ...
    return Bidi.isRtlLanguage(languageCode) ||
        F.of(context).isRTL(languageCode);
  }

  static String? convertDateTime(DateTime date) {
    return DateFormat.yMd().add_jm().format(date).replaceAll(' ', ' ');
  }
  ...
  static double calculateDistance(
    ...
    return distance.toInt().toDouble();
  }
  ...
  static String? getCurrencyCode(String? currency) {
    var currencies = currenciesData.toList();
