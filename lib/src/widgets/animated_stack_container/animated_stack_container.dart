...
class AnimatedStackContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget bodyWidget = _AnimatedStackContainer(
      ...
      clipBorderRadius:
          BorderRadius.circular(animatedConfig.clipBorderRadius ?? 0.0),
      ...
      decoration: animatedConfig.decoration != null
          ? BoxDecoration(
              ...
              boxShadow: animatedConfig.decoration?.boxShadows != null
                  ? [
                      BoxShadow(
                        color: (animatedConfig.decoration?.boxShadows?.color ??
                                const Color(0xFF000000))
                            .withValueOpacity(animatedConfig
                                    .decoration?.boxShadows?.opacity ??
                                1.0),
                        offset: Offset(
                        ...
                        blurRadius:
                            animatedConfig.decoration?.boxShadows?.blurRadius ??
                                0.0,
                        spreadRadius: animatedConfig
                                .decoration?.boxShadows?.spreadRadius ??
                            0.0,
                      ),
                    ]
                  : null,
      ...
      children: List.generate(
        animatedConfig.children.length,
        (index) {
  ...
  void _listenerController() {
    ...
    for (var i = 0; i < _listAnimatedUiItem.entries.length; i++) {
      ...
      for (var j = 0; j < item.value.length; j++) {
        ...
        if (isStart) {
          durationDelay =
              Duration(milliseconds: _maxDelay - durationDelay.inMilliseconds);
        }
