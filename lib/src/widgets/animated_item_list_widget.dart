...
class _AnimatedItemList extends State<AnimatedItemList>
  ...
  late final String _groupKey = widget.config.groupKey ??
    ...
    if (isCompleted) {
      ...
      if (_nextNode?.isCanForward ?? false) {
        ...
        if (_isScrollUp) {
          ...
          indexItemVisibleAndNotActive =
              _nodes.indexWhere((element) => element.isCanForward);
        } else {
          indexItemVisibleAndNotActive =
              _nodes.lastIndexWhere((element) => element.isCanForward);
        }
  ...
  void _handleNotFoundNode() {
    ...
    if (isAnyActive == false) {
      ...
      if (_isScrollUp) {
        final lastIndexVisible =
            _nodes.lastIndexWhere((element) => element.isVisible);
        _forwardAnimation(
            index: lastIndexVisible,
            labelDebug: 'handleNotFoundNode - scroll up');
      } else {
        // Scroll down, find the first visible node
        final indexFirstVisible =
            _nodes.indexWhere((element) => element.isVisible);
        _forwardAnimation(
  ...
  void initState() {
    ...
    _opacityAnimation = Tween<double>(begin: _enableAnimation ? 1 : 0, end: 0)
    ...
    return VisibilityDetector(
      ...
      onVisibilityChanged: (visibilityInfo) {
        ...
        if (_indexNode >= 0) {
          ...
          _nodes[_indexNode] =
              _nodes[_indexNode].copyWith(isVisible: _isVisible);
        }
      },
            ...
            for (var element in widget.config.animatedOptions) {
              switch (element.type) {
                ...
                case AniamtedOptionStyle.slideVertical:
                  ...
                  body = SlideTransition(
                    position: Tween<Offset>(
                            begin: Offset(dx, 0), end: const Offset(0, 0))
                        .animate(
                      CurvedAnimation(
...
class AnimatedOption {
  ...
  static List<AnimatedOption> get defaults {
    return [
      AnimatedOption.fadeIn(),
      AnimatedOption.slide(),
    ];
  }
}
