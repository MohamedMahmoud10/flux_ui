...
class FluxImage extends StatelessWidget {
  ...
  static void setConfig(
      {int? cacheImgeWidget, String? imageProxy, String? webProxy}) {
  ...
  FluxImage.fromConfig({
    super.key,
    required FluxImageConfig config,
  })  : imageUrl = config.url,
        ...
        alignment =
            config.styleConfig?.alignment ?? AlignmentDirectional.center,
  ...
  String _processImageUrl() {
    if (kIsWeb) {
      ...
      if (FluxImage.imageProxy.isNotEmpty) {
        imageProxy = '${FluxImage.imageProxy}${width}x,q30/';
      }
